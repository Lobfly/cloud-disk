<template>
  <div>
    <v-data-table :headers="headers" :items="pictrues" class="elevation-1" show-select :items-per-page="6" hide-default-footer :page.sync="page" @page-count="pageCount = $event">
      <!-- 图片 -->
      ...
      <template v-slot:[`item.file_size`]="{ item }">{{(item.file_size/104857).toFixed(2)+"Mb"}}</template>
      <!-- 按钮 -->
...
export default {
    ...
    methods:{
      ...
      getData(){
        _axios.defaults.headers.Authorization = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ0aW1lIjoxNjM0Mzg0NDIwODkxLCJ0aW1lb3V0Ijo4NjQwMDAwMCwidWlkIjoiMTYzNDEyNDI2Mjg3NSIsImlhdCI6MTYzNDM4NDQyMH0.743u49DLiKgM9u6OBU4Ul_WBd1eLIYnaCm-yaUgrQsc'
        _axios.get('/data/getImageData')
        .then(res=>{
          console.log(res.data)
    ...
    mounted(){
      this.getData()
      _axios.get('/data/getvideoData').then(res=>{console.log(res.data.data)})
    }
